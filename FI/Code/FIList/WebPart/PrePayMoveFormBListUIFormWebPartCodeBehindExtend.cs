...
{
    public partial class PrePayMoveFormBListUIFormWebPart
    {
		...
		private void BtnDelete_Click_Extend(object sender, EventArgs  e)
		{
			...
			BtnDelete_Click_DefaultImpl(sender,e);
				...
				//SaveClick154_Click...
		private void SaveClick154_Click_Extend(object sender, EventArgs  e)
		{
			//调用模版提供的默认实现.--默认实现可能会调用相应的Action.
			
		
			SaveClick154_Click_DefaultImpl(sender,e);
		}	
		...
		private void OnLookCase_Click_Extend(object sender, EventArgs  e)
		{
			...
			OnLookCase_Click_DefaultImpl(sender,e);
				...
				//OnOk214_Click...
		private void OnOk214_Click_Extend(object sender, EventArgs  e)
		{
			//调用模版提供的默认实现.--默认实现可能会调用相应的Action.
			
		
			OnOk214_Click_DefaultImpl(sender,e);
		}	
		 
				//OnClose217_Click...
		private void OnClose217_Click_Extend(object sender, EventArgs  e)
		{
			//调用模版提供的默认实现.--默认实现可能会调用相应的Action.
			
		
			OnClose217_Click_DefaultImpl(sender,e);
		}	
		...
		private void OnLoadData_Extend(object sender)
		{
            ...
            long prePayID = PubClass.GetLong(this.NameValues[Const_PrePayment]);

            if (prePayID > 0)
            {
                this.Model.PrePayment.CurrentFilter.OPath = string.Format("ID={0}",prePayID);

                this.Action.CommonAction.Load(this.Model.PrePayment, this.Model.PrePayment_PrePaymentLines, this.Model.PrePayment_PrePaymentLines_PrePaymentLineMoveDetails);
            }
