 ...
 ***********************************************************************************************/
namespace OrderBomChangeUIItemRef
{
    public partial class OrderBomChangeUIItemRefWebPart
    {
        ...
        private void ConfirmButton_Click_Extend(object sender, EventArgs e)
        {
            ...
            table.Columns.Add("SubKeyID", typeof(long));
            table.Columns.Add("SubKeyCode", typeof(string));
            table.Columns.Add("SubKeyName", typeof(string));
            table.Columns.Add("SoLineID", typeof(long));
            foreach (cRefRecord record in this.Model.cRef.SelectRecords)
            {

                DataRow dr = table.NewRow();
                //子项
                dr["SubKeyID"] = record.SubKey_ID;
                dr["SubKeyCode"] = record.SubKey_Code;
                dr["SubKeyName"] = record.SubKey_Name;
                //销售订单行ID
                dr["SoLineID"] = record.OrderLine_ID;
                table.Rows.Add(dr);

            }
            this.CurrentState["OrderBOMTable"] = table;
