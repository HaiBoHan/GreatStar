/**************************************************************
 * Description:
 * SynergyDetailBListUIModelAction.cs
 * Product: U9
 * Co.    : UFSoft Group
...
namespace UFIDA.U9.Cust.GS.FT.SynergyDetailBListUIModel
{
	public partial class SynergyDetailBListUIModelAction 
	{
...
this.OnOutPut_DefaultImpl(sender,e);								
		...
		private void OnGridRowDbClick_Extend(object sender, UIActionEventArgs e)
		{
            ...
            //是正拖的话，这里需用MainID
            string dataid = this.CurrentModel.CustomsDetailHead.FocusedRecord["ID"].ToString();
            if (!string.IsNullOrEmpty(dataid))
            {
                p.NameValues.Add("PDPageStatus", "browser");
                p.NameValues.Add("ID", dataid);
                UFIDA.U9.UI.PDHelper.NavigateManager.NavigatePage(this.CurrentPart, "Cust.GS.FT.SynergyDetailUIURI", p);
            }
...
this.OnGridRowDbClick_DefaultImpl(sender,e);								
		...
		private void OnNew_Extend(object sender, UIActionEventArgs e)
		{
            ...
            this.CurrentPart.NavigatePage("Cust.GS.FT.SynergyDetailUIURI", null);
