...
namespace UFIDA.U9.Cust.GS.FT.ShipCostForecastBE {
	...
	public partial class ShipCostForecast{
		...
		*/	
		...
		protected override void OnInserted() {
            using (ISession session = Session.Open())
            {
                ...
                {
                    ShipPlanDetailBE.ShipPlanDetailLine line = ShipPlanDetailBE.ShipPlanDetailLine.Finder.FindByID(this.ShipLine.ID);
                    if (line != null)
                        line.ForecastStateEnum = AllEnumBE.ForecastStateEnum.Y;
                }
                session.Commit();
            }
			base.OnInserted();
		...
		protected override void OnDeleted() {
            using (ISession session = Session.Open())
            {
                if (this.OriginalData.ShipPlan != null && this.OriginalData.ShipLine != null)
                {
                    ShipCostForecastBE.ShipCostForecast.EntityList lineList = ShipCostForecastBE.ShipCostForecast.Finder.FindAll("ShipLine=" + this.OriginalData.ShipLine.ID + "");
                      ShipPlanDetailBE.ShipPlanDetailLine line = ShipPlanDetailBE.ShipPlanDetailLine.Finder.FindByID(this.OriginalData.ShipLine.ID);
                      if (line != null)
                      {
                          if (lineList == null || lineList.Count == 0)
                          {
                              line.ForecastStateEnum = AllEnumBE.ForecastStateEnum.N;
                          }
                          else
                          {
                              line.ForecastStateEnum = AllEnumBE.ForecastStateEnum.Y;
		...
		private static ShipCostForecast CreateDefault_Extend() {
		...
		private  static ShipCostForecast CreateDefaultComponent_Extend(){
			//TODO delete next code and add your custome code here
			throw new NotImplementedException () ;
		}
