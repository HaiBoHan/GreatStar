...
namespace UFIDA.U9.Cust.GS.FT.BillNoItemBE {
	...
	public partial class BillNoItemLine{
		...
		*/	
        ...
        private void UpdateBillNoStatus(bool isDelete)
        {
            using (ISession session = Session.Open())
            {
                UFIDA.U9.Cust.GS.FT.BillNoBE.BillNo doc = UFIDA.U9.Cust.GS.FT.BillNoBE.BillNo.Finder.FindByID(this.BillNoid.ID);
                if (isDelete)
                {
                    doc.BillNoState = AllEnumBE.BillNoSatetEnum.UnCite;
                }
                else
                {
                    doc.BillNoState = AllEnumBE.BillNoSatetEnum.IsCite;
                }
                session.Commit();
            }
        }
		...
		private  static BillNoItemLine CreateDefault_Extend(UFIDA.U9.Cust.GS.FT.BillNoItemBE.BillNoItemHead parentEntity) {
