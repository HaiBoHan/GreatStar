...
{
	public partial class SynergyDetailUIModelAction 
	{
...
this.CopyClick_DefaultImpl(sender,e);								
		...
		private void SubmitClick_Extend(object sender, UIActionEventArgs e)
		{
            ...
            if (this.CurrentModel.CustomsDetailHead.FocusedRecord == null)
                return;
            long docKey = this.CurrentModel.CustomsDetailHead.FocusedRecord.ID;
            string strSysversion = this.CurrentModel.CustomsDetailHead.FocusedRecord.SysVersion.ToString();
            UFIDA.U9.Cust.GS.FT.SynergyStatusBP.Proxy.SynergyStatusBPProxy proxy = new SynergyStatusBP.Proxy.SynergyStatusBPProxy();
            proxy.SynergyDetail = docKey;
            proxy.TypeEunm = 1;
            proxy.OpType = 1;
            proxy.UISysVersion = strSysversion;
            proxy.Do();
...
this.OnClose_DefaultImpl(sender,e);								
		...
		private void UnApprovedClick_Extend(object sender, UIActionEventArgs e)
		{
			...
			//调用模版定义的默认实现方法.如需扩展,请直接在此编程.			
this.UnApprovedClick_DefaultImpl(sender,e);								
        } 
		private void CollectClick_Extend(object sender, UIActionEventArgs e)
		{

            if (this.CurrentModel.CustomsDetailHead.FocusedRecord == null)
                return;
            long detailHeadID = this.CurrentModel.CustomsDetailHead.FocusedRecord.ID;
            UFIDA.U9.Cust.GS.FT.SynergySplitBP.Proxy.SynergySplitProxy proxy = new UFIDA.U9.Cust.GS.FT.SynergySplitBP.Proxy.SynergySplitProxy();
            proxy.TrimLineID = detailHeadID;
            proxy.SrcDocTypeEnum = 2;
            proxy.Do();
            this.NavigateAction.Refresh(null);

			//调用模版定义的默认实现方法.如需扩展,请直接在此编程.			
this.CollectClick_DefaultImpl(sender,e);								
        } 
