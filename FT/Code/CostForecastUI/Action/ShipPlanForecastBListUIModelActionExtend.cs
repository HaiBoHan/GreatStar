/**************************************************************
 * Description:
 * ShipPlanForecastBListUIModelAction.cs
 * Product: U9
 * Co.    : UFSoft Group
...
{
	public partial class ShipPlanForecastBListUIModelAction 
	{
...
this.OnDelete_DefaultImpl(sender,e);								
		...
		private void OnRealTime_Extend(object sender, UIActionEventArgs e)
		{
			...
			//调用模版定义的默认实现方法.如需扩展,请直接在此编程.			
this.OnRealTime_DefaultImpl(sender,e);								
        } 
		private void OnDispatch_Extend(object sender, UIActionEventArgs e)
		{
			...
			//调用模版定义的默认实现方法.如需扩展,请直接在此编程.			
this.OnDispatch_DefaultImpl(sender,e);								
        } 
...
this.OnLoadData_DefaultImpl(sender,e);			
        ...
        private string CustomFilterOpath_Extend(string filterOpath)
        {
            ...
            {
                //已审核的出运明细单进行预测
                filterOpath += " org=" + UFIDA.U9.UI.PDHelper.PDContext.Current.OrgID + " and  Status=2";
                //已出运的出运明细单进行预测
                //filterOpath += " org=" + UFIDA.U9.UI.PDHelper.PDContext.Current.OrgID + " and  ShipPlanState !=0";
            }
            else
            {
                filterOpath += " and org=" + UFIDA.U9.UI.PDHelper.PDContext.Current.OrgID + " and Status=2 ";
                //filterOpath += " and org=" + UFIDA.U9.UI.PDHelper.PDContext.Current.OrgID + " and  ShipPlanState !=0";
            }
            return filterOpath;
        }
