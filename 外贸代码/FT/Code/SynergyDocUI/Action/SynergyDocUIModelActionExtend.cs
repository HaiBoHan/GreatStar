...
{
	public partial class SynergyDocUIModelAction 
	{
...
this.CopyClick_DefaultImpl(sender,e);								
		...
		private void SubmitClick_Extend(object sender, UIActionEventArgs e)
		{
            ...
            if (this.CurrentModel.SynergyHead.FocusedRecord == null)
                return;
            long docKey = this.CurrentModel.SynergyHead.FocusedRecord.ID;
            string strSysversion = this.CurrentModel.SynergyHead.FocusedRecord.SysVersion.ToString();
            UFIDA.U9.Cust.GS.FT.SynergyStatusBP.Proxy.SynergyStatusBPProxy proxy = new SynergyStatusBP.Proxy.SynergyStatusBPProxy();
            proxy.SynergyDocKey = docKey;
            proxy.TypeEunm = 0;
            proxy.OpType = 1;
