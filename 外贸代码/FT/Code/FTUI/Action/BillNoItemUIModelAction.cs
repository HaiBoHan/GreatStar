...
{
	public partial class BillNoItemUIModelAction : BaseAction
	{
		private static readonly ILogger logger = LoggerManager.GetLogger(typeof(BillNoItemUIModelAction));
		//强类型化基类Model属性.
		public new BillNoItemUIModelModel CurrentModel 
		{
			get {
				return (BillNoItemUIModelModel)base.CurrentModel;
			}
			set {
		...
		public BillNoItemUIModelAction(IPart part) : base(part)
		{
		}
		//Model参数的构造,用于测试用例.
		public BillNoItemUIModelAction(BillNoItemUIModelModel model) : base(model)
		{
		}
		/// <summary>
		/// Help: 
