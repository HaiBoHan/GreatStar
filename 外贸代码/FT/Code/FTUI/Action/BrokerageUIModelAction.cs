...
{
	public partial class BrokerageUIModelAction : BaseAction
	{
		private static readonly ILogger logger = LoggerManager.GetLogger(typeof(BrokerageUIModelAction));
		//强类型化基类Model属性.
		public new BrokerageUIModelModel CurrentModel 
		{
			get {
				return (BrokerageUIModelModel)base.CurrentModel;
			}
			set {
		...
		public BrokerageUIModelAction(IPart part) : base(part)
		{
		}
		//Model参数的构造,用于测试用例.
		public BrokerageUIModelAction(BrokerageUIModelModel model) : base(model)
		{
		}
		/// <summary>
		/// Help: 
